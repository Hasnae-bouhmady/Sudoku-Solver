#c'est le même sauf on a utiliser un fichier csv au lieu de l'import.
...
trainData = train.to_numpy()[0:,1:]
trainLabels = train._get_label_or_level_values("label")
testData = test.to_numpy()[0:,1:]
testLabels = test._get_label_or_level_values("label")
...
H = model.fit(trainData, trainLabels, validation_data=(testData, testLabels) , epochs=EPOCHS)
